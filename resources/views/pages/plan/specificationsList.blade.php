        ...
        @if(count($specifications) > 0)
            @foreach ($specifications as $spec)
                <div class="form-check" id="spec-{{$spec->id}}">
                    ...
                    <label class="form-check-label" for="{{$spec->id}}">{{$spec->spec_name}}</label>
                    <button type="button" class="btn text-warning edit-item" data-id="{{$spec->id}}" data-name="{{$spec->spec_name}}" data-toggle="modal" title="Edit"><i class="nav-icon i-pen-4"></i></button>
                    <button type="button" class="btn text-danger delete-item" data-id="{{$spec->id}}" data-name="{{$spec->spec_name}}" data-toggle="modal" title="Delete"><i class="nav-icon i-remove"></i></button>
                </div>
            @endforeach
        @endif
        </div>
